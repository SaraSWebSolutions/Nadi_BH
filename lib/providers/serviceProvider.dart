...
class ServiceListNotifier extends Notifier<List<Map<String, dynamic>>> {
  ...
  List<Map<String, dynamic>> build() {
    ...
    ref.listen<Locale>(languageProvider, (previous, next) {
      _loadServices(next.languageCode);
    });

    /// LOAD SERVICES
    Future.microtask(() async {
      await _loadServices(currentLang);
    });

    /// RETURN CACHED DATA IMMEDIATELY
    final cached = _box!.values
        .where((e) => e['lang'] == currentLang)
        .map((e) => Map<String, dynamic>.from(e))
        .toList();
  ...
  Future<void> _loadServices(String lang) async {
    ...
    try {
      ...
      /// DELETE OLD LANGUAGE SERVICES
      final keysToDelete = _box!.keys
          .where((key) => key.toString().startsWith('${lang}_'))
          .toList();
